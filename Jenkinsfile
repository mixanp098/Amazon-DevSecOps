pipeline {
    ...
    stages {
        ...
        stage("Git Checkout") {
            steps {
                git branch: 'main', url: 'http://10.129.200.216/mivadmsu/amazon-website.git'
            }
        }
        ...
        stage("Build Docker Image") {
            steps {
                script {
                    env.IMAGE_TAG = "mixanp098/amazon-website:${BUILD_NUMBER}"
        ...
        stage("Tag & Push to DockerHub") {
            steps {
                script {
                    withCredentials([string(credentialsId: 'docker-token', variable: 'dockerpwd')]) {
                        sh "docker login -u mixanp098 -p ${dockerpwd}"
                        sh "docker tag amazon ${env.IMAGE_TAG}"
                        sh "docker push ${env.IMAGE_TAG}"

                        // Also push latest image
                        sh "docker tag amazon mixanp098/amazon-website:latest"
                        sh "docker push mixanp098/amazon-website:latest"
                    }
                }
            }
        }
